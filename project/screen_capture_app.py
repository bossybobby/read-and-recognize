...
class ScreenCaptureApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.reader = ScreenCaptureReader(grid_shape=(8, 8), value_mode="gray")
    ...
    def _build_ui(self):
        ...
        self.mode_combo.addItems(["gray", "saturation", "value"])
    ...
    def update_frame(self):
        ...
        # --- 修复2: 正确调用识别器并增加调试 ---
        # 确保 grid 是正确的 (8, 8) numpy array
        try:
            # 调试：打印grid信息
            print(f"Debug - Grid shape: {grid.shape}, dtype: {grid.dtype}, min: {grid.min()}, max: {grid.max()}")
            
            # 确保是numpy array并且是float32 (preprocess_capacitive_data内部会处理归一化)
